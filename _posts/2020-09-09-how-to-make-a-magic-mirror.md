...
3. Flash the Raspberry Pi Raspbian Buster with Desktop image to your SD card with Etcher
  - *(Optional Step)* **If you don't have a Mini HDMI to HDMI adapter** or **if you don't want to configure the Pi via the desktop GUI** (i.e., a purely "headless" setup), you will want to add a couple of network configuration files to the `/boot` folder (accessible after you flash the image to the SD card) so that the Pi can connect to your network:
    - (1) `wpa_supplicant.conf` - the WiFi [network configuration file](/resources/01-MagicMirror/code/wpa_supplicant.txt)
...
- `sudo vim /home/pi/mmstart.sh`
  - [mmstart.sh](/resources/01-MagicMirror/code/mmstart_serveronly.sh.txt)
...
- `sudo vim /home/pi/mmstart.sh`
  - [mmstart.sh](/resources/01-MagicMirror/code/mmstart_serverclient.sh.txt)
...
- `sudo vim chromium_start.sh`
  - [chromium_start.sh](/resources/01-MagicMirror/code/chromium_start.sh.txt)
...
- `sudo vim /home/pi/motiondetectionscript.py`
  - [motiondetectionscript.py](/resources/01-MagicMirror/code/motiondetectionscript.py.txt)
...
As promised, here's my [MagicMirror config file](/resources/01-MagicMirror/code/config.js.txt) (with personal information stripped). Hope that it helps you!
